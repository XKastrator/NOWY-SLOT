...
const winLevelSoundsStop = () => {
	...
	if (stateBet.activeBetModeKey === 'superbonus' || stateGame.gameType === 'freegame') {
		// super bonus lub free spiny — muzyka free spinów
...
export const bookEventHandlerMap: BookEventHandlerMap<BookEvent, BookEventContext> = {
	...
	boardMultiplierInfo: async (bookEvent: BookEventOfType<'boardMultiplierInfo'>) => {
		...
		eventEmitter.broadcast({
			...
			totalMultiplier: bookEvent.winInfo.appliedMult ?? bookEvent.winInfo.boardMult,
		});
	...
	temperatureUpdate: async (bookEvent: BookEventOfType<'temperatureUpdate'>) => {
		// Reactor Temperature: persistent freegame multiplier, rendered via the
		// global multiplier UI.
		eventEmitter.broadcast({ type: 'globalMultiplierShow' });
		await eventEmitter.broadcastAsync({
			type: 'globalMultiplierUpdate',
			multiplier: bookEvent.temperature,
		});
	},
	mutation: async (bookEvent: BookEventOfType<'mutation'>) => {
		// Super Bonus: all instances of one low symbol mutate into a high symbol.
		eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_multiplier_win' });
		await animateSymbols({ positions: bookEvent.positions });
		const mutatedBoard = stateGameDerived
			.boardRaw()
			.map((reel) => reel.map((rawSymbol) => ({ ...rawSymbol })));
		bookEvent.positions.forEach((position) => {
			mutatedBoard[position.reel][position.row] = { name: bookEvent.targetSymbol };
		});
		eventEmitter.broadcast({ type: 'boardSettle', board: mutatedBoard });
		eventEmitter.broadcast({ type: 'boardShow' });
	},
	scatterPayInfo: async (bookEvent: BookEventOfType<'scatterPayInfo'>) => {
		// Direct scatter payout on a 4+ scatter trigger board.
		eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_scatter_win_v2' });
		const overlay = bookEvent.positions[Math.floor(bookEvent.positions.length / 2)];
		await Promise.all([
